...
base=`basename $i `
if [ -f $input_dir/model_$base ] ; then
	#$script_file./svm_learn -t 2 $i $output_dir/model_kernel2_$base
	$script_file./svm_classify $input_dir/BLAST_SVM_set$count.txt $input_dir/model_$base >$output_dir/accuracy_results_set$count.txt $output_dir/predicted_results_set$count.txt
	echo $base
